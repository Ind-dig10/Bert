...
train = pd.read_excel('data/train.xlsx') #pd.read_excel('data/train.xlsx') #
test = pd.read_excel('data/test.xlsx') #pd.read_excel('data/test.xlsx') #
...
class BertClassifier:
    def __init__(self, model_path, tokenizer_path, data, n_classes=6, epochs=4):
...
def inference(model, dataloader):
    ...
    with torch.no_grad():
        for test_input, test_oid in tqdm(dataloader):
            ...
            all_labels.extend(torch.argmax(output[0].softmax(0), dim=1))



            for prob in output[0].softmax(0):
                label_prob.append(prob)
        return ([oid.item() for oid in all_oid], [CLASSES[labels] for labels in all_labels], label_prob)


path_model2 = f'checkpoint_difficulty_level/BertClassifier{bert_tiny.epochs-1}.pt'

inference_model = torch.load(path_model2)
